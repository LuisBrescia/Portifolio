...
var temaOS = window.matchMedia("(prefers-color-scheme: dark)").matches
  ? "dark"
  : "light";
...
$(document).ready(function () {
  ...
  $(window).scroll(function () {
    ...
    switch (true) {
      case scrollTopParaExibir <= $("#experiencia").offset().top:
        if (scrollTopParaLink <= $("#projetos").offset().top) {
          break;
        }
  ...
  function alteraTema(tema) {
    $("#app").css({
      filter:
        "blur(5px) grayscale(100%) brightness(200%) contrast(200%) saturate(2)",
